...
> **Auth note — `context7`:** Registered at user scope via `claude mcp add` (writes to `~/.claude.json`), with `--env CONTEXT7_API_KEY="$CONTEXT7_API_KEY"` interpolated at startup. The key must be in your shell env (`~/.zshrc`: `export CONTEXT7_API_KEY=ctx7sk-…`). The `context7@claude-plugins-official` plugin registers its own MCP under namespace `mcp__plugin_context7_context7__*`, which is **unauthenticated** by default (Issue [#1713](https://github.com/upstash/context7/issues/1713)) and counts against the public free-tier pool. The user-scope `context7` registration is the authenticated one — quota appears on your Upstash dashboard. **Free tier:** 1,000 calls/month, 20 bonus calls/day after cap — avoid broad auto-invoke.

> **MCP registration mechanism — important note:** `~/.claude.json` (NOT `~/.claude/settings.json`'s `mcpServers` block) is the registry that `claude mcp list` actually reads on this user's Claude Code version. The `mcpServers` block in settings.json is preserved as defensive documentation (newer Claude Code releases may honor it), but the canonical install path is `scripts/bootstrap-mcps.sh`, which calls `claude mcp add --scope user`. **On a fresh machine: clone dotclaude, set `CONTEXT7_API_KEY` in `~/.zshrc`, then run `bash ~/.claude/scripts/bootstrap-mcps.sh`.** `~/.claude.json` is intentionally not versioned (it stores per-machine state).
