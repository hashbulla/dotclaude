...
| **Unipile (social ↔ agent bridge)** | `playbooks/unipile/` | 2026-06-30 | Wiring a Claude agent to send/receive across LinkedIn / WhatsApp / Telegram / Gmail via Unipile's unified messaging API. Account model (1 identity/network, 49€ ≤10), Hosted-Auth (session cookie never touches the agent), webhooks (`new_relation`/`new_message`) for receive, the cadence guardrail (relays platform limits, never lifts them — LinkedIn ban-risk), GDPR-FR (DPA Art. 28). Worked example: LinkedIn outreach. Creds: `~/second-brain/.secrets/unipile.env`. |
